...
def interpolate_inout_local(lcl_r_mb, lcl_q_mb, n_past, n_future):
    """
    ...
    n_trans = lcl_r_mb.shape[1] - (n_past + n_future) #  + n_future
    # interp_ws = np.linspace(0.0, 1.0, num=n_trans + 2, dtype=np.float32)
    interp_ws = np.arange(0, n_trans + 1 + n_future, 1) / (n_trans + 1)
    offset = end_lcl_r_mb - start_lcl_r_mb

    const_trans    = np.tile(start_lcl_r_mb, [1, n_trans + 1 + n_future, 1, 1])
...
def interpolate_global(glb_r_mb, glb_q_mb, start, end):
    """
    Performs interpolation between 2 frames of an animation sequence.
    ...
    :param glb_r_mb:  Local/Global root positions (B, T, J, 3)
    :param glb_q_mb:  Local quaternions (B, T, J, 4)
    :param n_past:    Number of frames of past context
    :param n_future:  Number of frames of future context
    :return: Interpolated root and quats
    """
    # Extract last past frame and target frame
    start_glb_r_mb = glb_r_mb[:, start, :, :][:, None, :, :]  # (B, 1, J, 3)
    end_glb_r_mb = glb_r_mb[:, end, :, :][:, None, :, :]

    if glb_q_mb is not None:
        start_glb_q_mb = glb_q_mb[:, start, :, :]
        end_glb_q_mb = glb_q_mb[:, end, :, :]

    # LERP Local Positions:
    n_trans = end - start - 1
    interp_ws = np.linspace(0.0, 1.0, num=n_trans + 2, dtype=np.float32)
    offset = end_glb_r_mb - start_glb_r_mb

    const_trans    = np.tile(start_glb_r_mb, [1, n_trans + 2, 1, 1])
    inter_glb_r_mb = const_trans + (interp_ws)[None, :, None, None] * offset

    # SLERP Local Quats:
    if glb_q_mb is not None:
        interp_ws = np.linspace(0.0, 1.0, num=n_trans + 2, dtype=np.float32)
        inter_glb_q_mb = np.stack(
            [(quat_normalize(quat_slerp(quat_normalize(start_glb_q_mb), quat_normalize(end_glb_q_mb), w))) for w in
            interp_ws], axis=1)
    else:
        inter_glb_q_mb = None
    return inter_glb_r_mb, inter_glb_q_mb
...
def single_rotate_at_frame_global(global_x, global_q, ref, target = None):
    """
    ...
    key_glob_Q = global_q[ref: ref + 1, 0:1, :]  # (B, 1, 1, 4)
    forward = np.array([1, 0, 1])[np.newaxis, np.newaxis, :] \
                 * quat_mul_vec(key_glob_Q, np.array([0, 1, 0])[np.newaxis, np.newaxis, :])
    forward = normalize(forward)
    yrot = quat_normalize(quat_between(target, forward))
    new_glob_Q = quat_mul(quat_inv(yrot), global_q)
    new_glob_X = quat_mul_vec(quat_inv(yrot), global_x)

    return new_glob_X, new_glob_Q
