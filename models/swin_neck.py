...
class SwinTransformerNeck(nn.Module):
    def __init__(self, in_dim, out_dim=256, depth=4, w_neck=True, drop_path_rate=0.1):
        super().__init__()
        ...
        if w_neck:
            ...
            self.stage = SwinBlockSequence(
                embed_dims=out_dim,
                num_heads=8,
                feedforward_channels=out_dim * 2,
                depth=depth,
                window_size=4,
                drop_path_rate=dpr,
                downsample=None
            ...
            self.norm_layer = build_norm_layer(dict(type='LN'), out_dim)[1]
    ...
    def forward(self, x):
        ...
        if self.w_neck:
            ...
            x, hw_shape, out, out_hw_shape = self.stage(x, hw)
            out = self.norm_layer(out)
            x_rep = out.view(-1, *out_hw_shape, self.out_dim).permute(0, 3, 1, 2).contiguous()
        else:
            x_rep = x
